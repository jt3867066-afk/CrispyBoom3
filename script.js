...
function sendOrderToWhatsApp(customer) {
    ...
    if (state.order.personal.quantity > 0) {
        message += `• ${state.order.personal.quantity}x Caja Personal (${state.order.personal.quantity * 2} choribolas)`;
...
// ==================== DETECCIÓN DE DOMINGO PARA PROMOCIONES ====================
function isSunday() {
...
if (isSunday()) {
    document.querySelector('.promo-text').style.display = 'block';
} else {
